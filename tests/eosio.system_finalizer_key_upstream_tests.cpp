// Ported from AntelopeIO/reference-contracts tests/eosio.finalizer_key_tests.cpp
// (the suite EOS mainnet's Savanna transition was validated with), adapted to
// the Telos system contract:
//  - fixture/suite renamed to avoid clashes with eosio.system_finalizer_key_tests.cpp
//  - get_row_by_id replaced with a get_row_by_account-based lookup
//  - switchtosvnn shortfall error differs: Telos's per-producer guard in
//    get_finalizers_for_producers fires before the aggregate count check
//  - update_elected_producers_finalizers_replaced_test rewritten for Telos
//    semantics (BP rotation above 21 candidates; schedule freezes if active
//    candidates drop below the last schedule size, instead of refilling)
...
struct finalizer_key_upstream_tester : eosio_system_tester {
   static const std::vector<key_pair_t> key_pair;

   fc::variant get_finalizer_key_info( uint64_t id ) {
      vector<char> data = get_row_by_account( config::system_account_name, config::system_account_name, "finkeys"_n, account_name(id) );
...
const std::vector<key_pair_t> finalizer_key_upstream_tester::key_pair {
...
BOOST_AUTO_TEST_SUITE(eosio_system_finalizer_key_upstream_tests)
...
BOOST_FIXTURE_TEST_CASE(register_finalizer_key_failure_tests, finalizer_key_upstream_tester) try {
...
BOOST_FIXTURE_TEST_CASE(register_finalizer_key_by_same_finalizer_tests, finalizer_key_upstream_tester) try {
   BOOST_REQUIRE_EQUAL( success(), regproducer(alice) );
...
BOOST_FIXTURE_TEST_CASE(register_finalizer_key_duplicate_key_tests, finalizer_key_upstream_tester) try {
   BOOST_REQUIRE_EQUAL( success(), regproducer(alice) );
...
BOOST_FIXTURE_TEST_CASE(register_finalizer_key_by_different_finalizers_tests, finalizer_key_upstream_tester) try {
...
BOOST_FIXTURE_TEST_CASE(register_duplicate_key_from_different_finalizers_tests, finalizer_key_upstream_tester) try {
   BOOST_REQUIRE_EQUAL( success(), regproducer(alice) );
   BOOST_REQUIRE_EQUAL( success(), regproducer(bob) );
...
BOOST_FIXTURE_TEST_CASE(activate_finalizer_key_failure_tests, finalizer_key_upstream_tester) try {
   // bob111111111 does not have Alice's authority
   BOOST_REQUIRE_EQUAL( error( "missing authority of bob111111111" ),
...
BOOST_FIXTURE_TEST_CASE(activate_finalizer_key_success_tests, finalizer_key_upstream_tester) try {
...
BOOST_FIXTURE_TEST_CASE(delete_finalizer_key_failure_tests, finalizer_key_upstream_tester) try {
   // bob111111111 does not have Alice's authority
   BOOST_REQUIRE_EQUAL( error( "missing authority of bob111111111" ),
...
BOOST_FIXTURE_TEST_CASE(delete_finalizer_key_success_test, finalizer_key_upstream_tester) try {
   // Alice registers as a producer
   BOOST_REQUIRE_EQUAL( success(), regproducer(alice) );
...
BOOST_FIXTURE_TEST_CASE(delete_last_finalizer_key_test, finalizer_key_upstream_tester) try {
   // Alice registers as a producer
   BOOST_REQUIRE_EQUAL( success(), regproducer(alice) );
...
BOOST_FIXTURE_TEST_CASE(switchtosvnn_success_tests, finalizer_key_upstream_tester) try {
...
BOOST_FIXTURE_TEST_CASE(multi_activation_tests, finalizer_key_upstream_tester) try {
   // Register and vote 26 producers
   auto producer_names = active_and_vote_producers();
   // Register 21 finalizer keys for the first 21 producers
   register_finalizer_keys(producer_names, 21);
...
BOOST_FIXTURE_TEST_CASE(switchtosvnn_missing_authority_tests, finalizer_key_upstream_tester) try {
   BOOST_REQUIRE_EQUAL( error( "missing authority of eosio" ),
                        push_action( alice, "switchtosvnn"_n, mvo()) );
}
FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE(switchtosvnn_not_enough_finalizer_keys_tests, finalizer_key_upstream_tester) try {
   auto producer_names = active_and_vote_producers();
   ...
   BOOST_REQUIRE_EQUAL( wasm_assert_msg( "producer defproduceru does not have an active finalizer key" ),
                        push_action( config::system_account_name, "switchtosvnn"_n, mvo()) );
}
FC_LOG_AND_RETHROW()

// Finalizers are not changed in current schedule rounds
BOOST_FIXTURE_TEST_CASE(update_elected_producers_no_finalizers_changed_test, finalizer_key_upstream_tester) try {
   auto producer_names = active_and_vote_producers();
   register_finalizer_keys(producer_names, 21);
   BOOST_REQUIRE_EQUAL(success(),  push_action( config::system_account_name, "switchtosvnn"_n, mvo()) );
...
BOOST_FIXTURE_TEST_CASE(update_elected_producers_finalizers_changed_test, finalizer_key_upstream_tester) try {
   auto producer_names = active_and_vote_producers();
   register_finalizer_keys(producer_names, 21);
   BOOST_REQUIRE_EQUAL(success(),  push_action( config::system_account_name, "switchtosvnn"_n, mvo()) );
...
// Telos adaptation of upstream's "finalizer replaced" test. Under Telos,
// if the keyed candidate count drops below the last schedule size the
// schedule and finalizer policy freeze rather than refilling from standbys,
// and voting more than 21 producers engages BP rotation. So the replacement
// flow is: add a NEW keyed producer first, then delete an existing
// finalizer's only key, leaving exactly 21 keyed candidates, and verify the
// policy converges to include the new producer and exclude the deleted one.
BOOST_FIXTURE_TEST_CASE(update_elected_producers_finalizers_replaced_test, finalizer_key_upstream_tester) try {
   auto producer_names = active_and_vote_producers();
   register_finalizer_keys(producer_names, 21);
   BOOST_REQUIRE_EQUAL(success(), push_action(config::system_account_name, "switchtosvnn"_n, mvo()));
   ...
   // defproducerv: a new producer with a registered finalizer key
   account_name producerv = "defproducerv"_n;
   setup_producer_accounts({producerv}, core_sym::from_string("1000.0000"));
   BOOST_REQUIRE_EQUAL( success(), regproducer(producerv) );
   BOOST_REQUIRE_EQUAL( success(), register_finalizer_key(producerv, finalizer_key_1, pop_1) );
   uint64_t producerv_id = get_finalizer_info(producerv)["active_key_id"].as_uint64();

   // vote it in alongside the original 21
   std::vector<name> new_votes = producer_names;
   new_votes.push_back(producerv);
   std::sort(new_votes.begin(), new_votes.end());
   BOOST_REQUIRE_EQUAL( success(), vote("alice1111111"_n, new_votes) );

   // defproducera deletes its only finalizer key, losing schedule eligibility;
   // exactly 21 keyed candidates remain
   name producera = "defproducera"_n;
   uint64_t deleted_id = get_finalizer_info(producera)["active_key_id"].as_uint64();
   auto deleted_key_str = get_finalizer_key_info(deleted_id)["finalizer_key"].as_string();
   BOOST_REQUIRE_EQUAL( success(), delete_finalizer_key(producera, deleted_key_str) );

   produce_blocks(504);

   auto last_finkey_ids_2 = get_last_prop_fin_ids();
   BOOST_REQUIRE_EQUAL( 21u, last_finkey_ids_2.size() );
   BOOST_REQUIRE_EQUAL( 1u, last_finkey_ids_2.count(producerv_id) );
   BOOST_REQUIRE_EQUAL( 0u, last_finkey_ids_2.count(deleted_id) );
}
FC_LOG_AND_RETHROW()
